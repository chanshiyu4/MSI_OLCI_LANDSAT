# Multi-Sensor Lake Pipeline (MSI + OLCI + Landsat)

This folder now supports one unified queue pipeline for:

1. Sentinel-2 MSI (CDSE)
2. Sentinel-3 OLCI (CDSE)
3. Landsat 8/9 (USGS M2M)

Core scripts:

- `monitor_producer.py`: unified producer (plugin-style provider strategy)
- `worker_acolite.py`: unified download + ACOLITE worker
- `worker_postprocess.py`: unified mosaic/postprocess worker

## Architecture

- Producer uses provider plugins:
  - `providers/cdse_provider.py`
  - `providers/usgs_provider.py`
- Worker receives standardized task JSON and handles provider-specific download.
- Postprocess is provider-agnostic and only depends on GeoTIFF outputs.
  ...
  "lakes": [
    {
      "uid": "Dianchi",
      "name": "Dianchi",
      "bbox": [102.55, 24.60, 102.85, 25.05]
    }
  ]
}
```

No shapefile is required. If `MASK_SHP_DIR/<uid>.shp` does not exist, postprocess outputs rectangular extent TIFFs.

## Required Environment Variables

For Sentinel (MSI/OLCI):
...
- `ENABLED_SENSOR_KEYS` (default: `MSI,OLCI,LANDSAT`)
- `MONITOR_LOOKBACK_DAYS` (default: `3`)
- `RABBITMQ_HOST` (default: `localhost`)
- `LAKES_CONFIG_PATH` (default: `MSI_OLCI_Landsat/lakes.json`)
- `TEMP_DIR`, `ACOLITE_OUT_DIR`, `FINAL_OUTPUT_DIR`, `MASK_SHP_DIR`
- `ACOLITE_PYTHON`, `ACOLITE_CLI_PATH`
- `CLEAN_ACOLITE_TEMP` (`1/true` to remove ACOLITE outputs after postprocess)
...
Start only producer with selected sensors:
